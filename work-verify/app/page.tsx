...
// The specific token we want to track
...
export default function VerifyPage() {
  ...
  const fetchTokenBalance = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.error("Error fetching token accounts, retrying once:", tokenErr);
      ...
      if (specificTokenAccount) {
        ...
        setTokenBalance(null);
        setError(`Token ${SPECIFIC_TOKEN_MINT.slice(0, 6)}...${SPECIFIC_TOKEN_MINT.slice(-6)} not found in this wallet`);
      }
    } catch (err) {
      ...
      if (err instanceof Error) {
        if (err.message.includes("timeout")) {
          setError("Connection timeout while fetching token balance. Network may be congested.");
  ...
  useEffect(() => {
    fetchTokenBalance();
  }, [fetchTokenBalance, publicKey, connected]);
  ...
  }, [tokenBalance, verificationResult]);
