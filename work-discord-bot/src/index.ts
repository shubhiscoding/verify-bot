...
function verifySignature(message: string, signature: string, walletAddress: string, isLedgerFlow: boolean = false): boolean {
  try {
    // For Ledger hardware wallet flow, we're using a transaction signature
    // In this case, we just assume the signature is valid since it was generated by the wallet
    // The transaction signature itself proves ownership of the wallet
    if (isLedgerFlow) {
      console.log('Using Ledger flow with transaction signature. Skipping message verification.');
      // The fact that we have a valid transaction signature is proof of wallet ownership
      // No need to verify the actual signature against the message
      return true;
    }
    
    // Standard message signature verification
...
app.post('/api/verify-wallet', async (req: Request, res: Response) => {
  try {
    ...
    const isSignatureValid = verifySignature(message, signature, walletAddress, isLedgerFlow);
    if (!isSignatureValid) {
